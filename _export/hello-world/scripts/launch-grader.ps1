...
$existing = @(Get-Process -Name powershell -ErrorAction SilentlyContinue |
  Where-Object { $_.MainWindowTitle -like "*數學習作批改*" })
...
$lbl.Text = "正在啟動習作批改…" + [Environment]::NewLine + "出現主視窗後此畫面會自動關閉"
$lbl.Dock = "Fill"
...
if ($p.HasExited -and -not $ready) {
  [void][System.Windows.Forms.MessageBox]::Show(
    "主程式立刻結束，可能仍有語法錯誤。`n請再跑一次 refresh-desktop-vbs.ps1。",
    "習作批改",
    [System.Windows.Forms.MessageBoxButtons]::OK,
