...
try {
  ...
  foreach ($sub in @('掃描匯入', '匯出給手機', 'scans-in', 'export-phone')) {
...
"@
  ...
  $msg = "習作台啟動失敗（已寫入桌面「TeacherDesk-error.txt」）`r`n`r`n$($_.Exception.Message)`r`n`r`n$($_.ScriptStackTrace)"
  Show-Fail $msg
  exit 1
}
