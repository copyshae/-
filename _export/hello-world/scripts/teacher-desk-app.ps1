...
if (-not $WorkDir) {
  ...
  $zh = Join-Path $desk '習作台資料'
  $legacy = Join-Path $desk 'TeacherDesk'
  if (Test-Path -LiteralPath $preferred) { $WorkDir = $preferred }
  elseif (Test-Path -LiteralPath $zh) { $WorkDir = $zh }
  elseif (Test-Path -LiteralPath $legacy) { $WorkDir = $legacy }
  else { $WorkDir = $preferred }
}
...
New-Item -ItemType Directory -Force -Path (Join-Path $WorkDir 'scans-in') | Out-Null
New-Item -ItemType Directory -Force -Path (Join-Path $WorkDir 'export-phone') | Out-Null
...
$lblSub.Text = '完整版：掌握／發送／篩選／管道／LINE／同步／掃描夾（與手機互通）'
...
[void]$form.ShowDialog()
  ...
  $msg = "習作台啟動失敗（已寫入桌面 TeacherDesk-error.txt）：`n$($_.Exception.Message)`n`n$($_.ScriptStackTrace)"
  try {
    $logPath = Join-Path ([Environment]::GetFolderPath('Desktop')) 'TeacherDesk-error.txt'
    $enc = New-Object System.Text.UTF8Encoding $true
    [System.IO.File]::WriteAllText($logPath, $msg, $enc)
  } catch {}
  try {
