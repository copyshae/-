...
Write-Host "  TeacherDesk-start.cmd　＝　習作台（最穩，建議用這個）"
Write-Host "  習作台.cmd／習作台.vbs　＝　同上"
Write-Host "  習作批改.vbs　　　　　　＝　批閱習作／產出練習（若已安裝）"
Write-Host "工作資料夾："
Write-Host "  桌面\TeacherDeskData\"
Write-Host "  桌面\MathGrading\（習作批改用，內含中文子資料夾）"
Write-Host "若一閃就沒：看桌面 TeacherDesk-error.txt，並重跑 install-teacher-desk.ps1"
