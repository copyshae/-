...
#
# Usage (run once on each Windows computer):
#   cd <hello-world repo>
...
#
# 手機 Cursor：不要跑本腳本。Cloud Agent 看不到 %USERPROFILE%\.cursor。
# 請用手機 Cursor 開 copyshae/- 或本倉庫，直接打「推日誌」「連日誌」。
# 說明頁：directory/logs/prompts/
...
Write-Host ''
Write-Host 'Reopen a Cursor chat, then type: 推日誌'
Write-Host 'Desktop apps shortcut: 裝習作台和習作批改'
Write-Host 'Phone Cursor: open copyshae/- and type the same phrases (no install script).'
